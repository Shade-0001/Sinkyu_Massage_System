...
class PrintsController extends Controller
{
  ...
  /**
   * 総括表用：施術タイプ別にデータが存在する年月と件数のマップを返す
   *
  ...
  protected function getSummaryTableDataMonths(): array
  {
    ...
    $acupunctureMap = [];
    foreach ($acupunctureRows as $row) {
      $acupunctureMap[$row->ym] = (int)$row->cnt;
    }

    $massageMap = [];
    foreach ($massageRows as $row) {
      $massageMap[$row->ym] = (int)$row->cnt;
    }

    return [
      'acupuncture' => $acupunctureMap,
      'massage'     => $massageMap,
    ];
  }
  ...
  public function referrerThankYou(Request $request, string $filename)
  {
    ...
    try {
      \Log::info('紹介者への御礼状PDF生成開始', [
        'clinic_user_ids' => $validated['clinic_user_ids'],
        'caremanager_id'  => $validated['caremanager_id'],
        'submission_date' => $validated['submission_date'],
      ]);

      $service = new \App\Services\Print\ThankYouLetterReferrerPdfService();
      ...
      $pdfBinary = $service->generate(
        ...
        $validated['submission_date'],
        '',
        [$validated['caremanager_id']]
      );

      \Log::info('紹介者への御礼状PDF生成完了', ['size' => strlen($pdfBinary)]);
   ...
   */
  public function userInfoInsuranceList(Request $request, string $filename)
  {
    try {
      $service    = new \App\Services\Print\ClinicUserInsuranceInfoListPdfService();
      $outputDate = now()->format('Y-m-d');

      \Log::info('利用者情報一覧（医療保険情報）PDF生成開始');

      $pdfBinary = $service->generate([], '', $outputDate);

      \Log::info('利用者情報一覧（医療保険情報）PDF生成完了', ['size' => strlen($pdfBinary)]);
      ...
      \Log::error('利用者情報一覧（医療保険情報）PDF生成エラー', [
        'message' => $e->getMessage(),
        'file'    => $e->getFile(),
        'line'    => $e->getLine(),
        'trace'   => $e->getTraceAsString(),
   ...
   */
  public function clinicUserConsentInfoList(Request $request, string $filename)
  {
    try {
      $service    = new \App\Services\Print\ClinicUserConsentInfoListPdfService();
      $outputDate = now()->format('Y-m-d');

      \Log::info('利用者情報一覧（同意医師情報）PDF生成開始');

      $pdfBinary = $service->generate([], '', $outputDate);

      \Log::info('利用者情報一覧（同意医師情報）PDF生成完了', ['size' => strlen($pdfBinary)]);
      ...
      \Log::error('利用者情報一覧（同意医師情報）PDF生成エラー', [
        'message' => $e->getMessage(),
        'file'    => $e->getFile(),
        'line'    => $e->getLine(),
        'trace'   => $e->getTraceAsString(),
   ...
   * 利用者情報一覧（基本情報）PDF出力
   */
  public function userInfoBasicList(Request $request, string $filename)
  {
    try {
      $service    = new \App\Services\Print\UserInfoBasicListPdfService();
      $outputDate = now()->format('Y-m-d');

      \Log::info('利用者情報一覧（基本情報）PDF生成開始');

      $pdfBinary = $service->generate([], '', $outputDate);

      \Log::info('利用者情報一覧（基本情報）PDF生成完了', ['size' => strlen($pdfBinary)]);
      ...
      \Log::error('利用者情報一覧（基本情報）PDF生成エラー', [
        'message' => $e->getMessage(),
        'file'    => $e->getFile(),
        'line'    => $e->getLine(),
        'trace'   => $e->getTraceAsString(),
   ...
   * 医師情報一覧PDF出力
   */
  public function doctorInfoList(Request $request, string $filename)
  {
    try {
      $service    = new \App\Services\Print\DoctorInfoListPdfService();
      $outputDate = now()->format('Y-m-d');

      \Log::info('医師情報一覧PDF生成開始');

      $pdfBinary = $service->generate([], '', $outputDate);

      \Log::info('医師情報一覧PDF生成完了', ['size' => strlen($pdfBinary)]);
  ...
  public function careManagerInfoList(Request $request, string $filename)
  {
    try {
      $service    = new \App\Services\Print\CareManagerInfoListPdfService();
      $outputDate = now()->format('Y-m-d');

      \Log::info('ケアマネ情報一覧PDF生成開始');

      $pdfBinary = $service->generate([], '', $outputDate);

      \Log::info('ケアマネ情報一覧PDF生成完了', ['size' => strlen($pdfBinary)]);
  ...
  public function weeklySchedule(Request $request, string $filename)
  {
    ...
    try {
      $service = new \App\Services\Print\WeeklySchedulePdfService();

      \Log::info('週間スケジュールPDF生成開始', [
        'week_start_date' => $validated['week_start_date'],
        'therapist_id'    => $validated['therapist_id'] ?? null,
      ]);

      $pdfBinary = $service->generateWeekly(
        $validated['week_start_date'],
        $validated['therapist_id'] ?? null
      );

      \Log::info('週間スケジュールPDF生成完了', ['size' => strlen($pdfBinary)]);
