 ...
 * ============================================================
 * このサービスは document_id_1 = 5（医師への御礼状（同意書発行））
 * の文面を参照する。
 *
 * DocumentAssociationController.phpのfixedDocuments定義：
 * - id=5: 医師への御礼状（同意書発行） ← このサービスが参照
 * - id=6: 医師への御礼状（一般）
 * - id=7: 紹介者への御礼状
 *
...
class ThankYouLetterDoctorPdfService extends BasePdfService
{
  ...
  protected function getSampleData(string $submissionDate): array
  {
    ...
    \Log::info('ThankYouLetterDoctor getSampleData実行', [
    ...
    $documentContent = $custom['document_content'] ?? "拝啓　時下ますますご清栄のこととお慶び申し上げます。\n\nさて、このたびは下記の方に対しまして、はり・きゅう施術の同意書をご交付いただきまして、誠にありがとうございました。\nおかげさまで施術を進めることができております。今後とも変わらぬご支援をお願い申し上げます。\n\n敬具";
