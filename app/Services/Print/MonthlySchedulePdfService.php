...
class MonthlySchedulePdfService extends BasePdfService
{
  ...
  protected function drawTable(
    ...
    $this->drawSlotGridAndTimeLabels($pdf, $timeSlots, $colWidths, $startX, $bodyStartY, $rowH, $dayCount);
  ...
  protected function drawEventRect(
    ...
    if ($use3Lines) {
      ...
      // 2行モード：時刻テキストが $innerW にぴったり収まる最大フォントサイズを決定
      $timeText  = $startText . ' - ' . $endText;
      $maxFontSize = 20.0; // 上限
      // まず縦・横両方収まる最大フォントサイズを上から探す
