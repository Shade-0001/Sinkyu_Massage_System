 ...
 */
class MedicalAssistanceMassagePdfService
{
  /**
   * 座標設定
   */
  ...
  {
    $configPath = $this->customCoordinatesPath ?? storage_path('app/config/medical_assistance_massage_coordinates.json');
  ...
  protected function fillFormFields(Fpdi $pdf, array $data, string $submissionDate): void
  {
    ...
    if ($this->sampleDataMode) {
      if (isset($this->coordinates['bill_category_new']['isSelected']) && $this->coordinates['bill_category_new']['isSelected']) {
        $pdf->SetFontSize($this->coord('bill_category_new', 'fontSize'));
        $this->drawTextByKey($pdf, 'bill_category_new', '新規');
        $pdf->SetFontSize(10);
      } elseif (isset($this->coordinates['bill_category_continued']['isSelected']) && $this->coordinates['bill_category_continued']['isSelected']) {
        $pdf->SetFontSize($this->coord('bill_category_continued', 'fontSize'));
        $this->drawTextByKey($pdf, 'bill_category_continued', '継続');
        $pdf->SetFontSize(10);
    ...
    if ($this->sampleDataMode) {
      if (isset($this->coordinates['outcome_continued']['isSelected']) && $this->coordinates['outcome_continued']['isSelected']) {
        $pdf->SetFontSize($this->coord('outcome_continued', 'fontSize'));
        $this->drawTextByKey($pdf, 'outcome_continued', '継続');
        $pdf->SetFontSize(10);
      } elseif (isset($this->coordinates['outcome_cured']['isSelected']) && $this->coordinates['outcome_cured']['isSelected']) {
        $pdf->SetFontSize($this->coord('outcome_cured', 'fontSize'));
        $this->drawTextByKey($pdf, 'outcome_cured', '治癒');
        $pdf->SetFontSize(10);
      } elseif (isset($this->coordinates['outcome_discontinued']['isSelected']) && $this->coordinates['outcome_discontinued']['isSelected']) {
        $pdf->SetFontSize($this->coord('outcome_discontinued', 'fontSize'));
        $this->drawTextByKey($pdf, 'outcome_discontinued', '中止');
        $pdf->SetFontSize(10);
      } elseif (isset($this->coordinates['outcome_transferred']['isSelected']) && $this->coordinates['outcome_transferred']['isSelected']) {
        $pdf->SetFontSize($this->coord('outcome_transferred', 'fontSize'));
        $this->drawTextByKey($pdf, 'outcome_transferred', '転医');
        $pdf->SetFontSize(10);
        ...
        if ($key) {
          ...
          $this->drawTextByKey($pdf, $key, $this->customSampleData['outcome']);
          $pdf->SetFontSize(10);
        }
      }
      ...
      if ($key) {
        $pdf->SetFontSize($this->coord($key, 'fontSize'));
        $this->drawTextByKey($pdf, $key, $consent->outcome);
        $pdf->SetFontSize(10);
      }
    }
