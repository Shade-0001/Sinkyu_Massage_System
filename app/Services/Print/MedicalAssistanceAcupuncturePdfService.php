...
/**
 * はり・きゅう医療助成費支給申請書PDF生成サービス
 */
class MedicalAssistanceAcupuncturePdfService
{
  /**
   * 座標設定
   */
  ...
  {
    $configPath = $this->customCoordinatesPath ?? storage_path('app/config/medical_assistance_acupuncture_coordinates.json');
  ...
  protected function getDefaultCoordinates(): array
  {
    ...
    $configPath = storage_path('app/config/medical_assistance_acupuncture_coordinates.json');
    $json = file_get_contents($configPath);
  ...
  protected function fillFormFields(Fpdi $pdf, array $data, string $submissionDate): void
  {
    ...
    // === 被保険者証記号・番号 ===
    // 保険種別1によって表示形式が異なる
    // 社・国・組(ID:1): 記号(code_number)と番号(account_number)を別々のフィールドに表示
    // 公費(ID:2), 後期(ID:3), 退職(ID:4): 被保険者番号(insured_number)を番号フィールドに表示
    if ($insurance) {
      ...
      if ($insuranceType1Id == 1) {
        ...
        if (!$symbol && !$number) {
          \Log::warning('被保険者記号・番号が設定されていません', [
            'insurance_type_1_id' => $insuranceType1Id,
            'insurance' => $insurance
          ]);
        }
      } else {
        // 公費・後期・退職の場合: 被保険者番号を番号フィールドに表示
        $insuredNumber = $insurance->insured_number ?? '';

        if ($insuredNumber) {
          $pdf->SetFontSize($this->coord('insurance_symbol_number', 'fontSize'));
          $this->drawTextByKey($pdf, 'insurance_symbol_number', (string)$insuredNumber);
          $pdf->SetFontSize(10);
        } else {
          \Log::warning('被保険者番号が設定されていません', [
            'insurance_type_1_id' => $insuranceType1Id,
            'insurance' => $insurance
          ]);
        }
