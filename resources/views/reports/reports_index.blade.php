    ...
    <div id="reports-list-area" style="max-height: 70vh; overflow-y: auto; overflow-x: hidden; border: 1px solid #dee2e6;">
      ...
      @foreach ($reportsByYear as $year => $yearData)
        ...
        <div class="year-header mb-2 d-flex align-items-center">
          <button class="btn-ex btn-ex-sub btn-ex-blue btn-ex-xl btn-ex-sub-toggle-invert fs-2 gap-2" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $collapseId }}" aria-expanded="{{ $isYearExpanded ? 'true' : 'false' }}" aria-controls="{{ $collapseId }}">
        ...
        <div class="collapse {{ $isYearExpanded ? 'show' : '' }}" id="{{ $collapseId }}" data-year="{{ $year }}">
          @foreach ($months as $item)
            ...
            <div class="report-month-section mb-4 ms-4" data-year-month="{{ $yearMonth }}">
              @if ($item['report'])
                <!-- 報告書データあり -->
                <div class="btn-ex btn-ex-sub btn-ex-blue btn-ex-lg btn-ex-sub-toggle-invert" role="button" data-bs-toggle="collapse" data-bs-target="#{{ $monthCollapseId }}" aria-expanded="{{ $isMonthExpanded ? 'true' : 'false' }}" aria-controls="{{ $monthCollapseId }}">
