            ...
            <div class="mb-3">
              ...
              <select class="form-select" id="clinic_user_ids" name="clinic_user_ids[]" multiple size="10" required>
                @foreach($clinicUsers as $user)
                  <option value="{{ $user->id }}">{{ $user->last_name }}{{ "\u{2000}" }}{{ $user->first_name }}｜{{ $user->last_kana }}{{ "\u{2000}" }}{{ $user->first_kana }}｜ID-{{ str_pad($user->id, $clinicUserIdLength, '0', STR_PAD_LEFT) }}</option>
                @endforeach
              </select>
          ...
          <form id="implementationPlanForm" method="POST">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" id="implementation_plan_clinic_user_ids" name="clinic_user_ids[]" multiple size="10" required>
                @foreach($clinicUsers as $user)
                  <option value="{{ $user->id }}" data-plan-months="{{ json_encode($implementationPlanUserMonths[$user->id] ?? []) }}">{{ $user->last_name }}{{ "\u{2000}" }}{{ $user->first_name }}｜{{ $user->last_kana }}{{ "\u{2000}" }}{{ $user->first_kana }}｜ID-{{ str_pad($user->id, $clinicUserIdLength, '0', STR_PAD_LEFT) }}</option>
                @endforeach
              </select>
              <div class="form-text fw-medium">複数選択可（クリックで選択/解除、長押し+ドラッグで連続選択）</div>
            </div>
