            ...
            <div class="mb-3">
              ...
              <select class="form-select" id="clinic_user_ids" name="clinic_user_ids[]" multiple size="10" required>
                @foreach($clinicUsers as $user)
                  <option value="{{ $user->id }}">{{ str_repeat("\u{00A0}", max(0, 3 - strlen((string)$user->id))) . '［' . $user->id . '］' . $user->last_name . "\u{00A0}\u{00A0}" . $user->first_name . '（' . $user->last_kana . "\u{00A0}\u{00A0}" . $user->first_kana . '）' }}</option>
                @endforeach
              </select>
