...
## 〈 コーディング関連ルール 〉
### 共通事項
- ファイルの序盤にメモが記載されている場合は、必ずそのメモをチェックして。

### タスク完了時の自動コミット（目的：AIｺｰﾃﾞｨﾝｸﾞｴｰｼﾞｪﾝﾄによる変更を後から任意の段階まで巻き戻せるようにするため）
- Gitを利用したプロジェクトに変更を加える場合は、各タスク完了後に毎回必ず以下のStepを実行して。
...
### 通常のコミット
- ユーザーから直接指示を受けてコミットを実行する場合は、コミットメッセージ形式を "［normal-commit_YYMMDD-HHMM｜タスク内容の要約］"　（先頭と末尾に［］を付けて）（YYMMDD-HHMMのYYは西暦の下2ケタ）として。
