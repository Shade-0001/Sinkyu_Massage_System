            ...
            if (mapping?.hidden) {
              ...
              if (currentPdfType === 'therapy_benefit_massage') {
                if (field.compositeGroup === 'first_treatment_date_composite') {
                  ...
                  if (k === 'first_treatment_era' ||
                      (k.startsWith('first_treatment_') && (k.endsWith('_year') || k.endsWith('_month') || k.endsWith('_day')))) {
                    return true;
                  }
                  return false;
                }
              ...
              if ((currentPdfType === 'medical_assistance_acupuncture' || currentPdfType === 'medical_assistance_massage' || currentPdfType === 'elderly_therapy_benefit_acupuncture' || currentPdfType === 'elderly_therapy_benefit_massage') &&
                  (field.compositeGroup === 'first_treatment_date_composite' ||
                   ...
                   field.compositeGroup === 'treatment_end_date_composite')) {
                // 日付フィールド（*_date）のみ表示
                if (k === 'first_treatment_date' || k === 'treatment_start_date' || k === 'treatment_end_date') {
                  return true;
                }
                return false;
              }
      ...
      if (fieldMapping?.radioGroup && !processedGroups.has(fieldMapping.radioGroup)) {
        ...
        const groupFields = Object.entries(coordinates)
          .filter(([k, v]) => {
            ...
            return km?.radioGroup === fieldMapping.radioGroup;
          })
          .sort((a, b) => {
            const indexA = orderedKeys.indexOf(a[0]);
            const indexB = orderedKeys.indexOf(b[0]);
        ...
        div.innerHTML = `
          ...
          <div class="field-controls" id="controls-${fieldMapping.radioGroup}">
            ...
            <div id="radiogroup-fields-${fieldMapping.radioGroup}">
